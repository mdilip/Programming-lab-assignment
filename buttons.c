		     ...
		     gint C, gint D, gint E )
{
  ...
  gtk_box_pack_start (GTK_BOX (Hbox), button, C, D, E);
  gtk_widget_show (button);
  g_signal_connect_swapped(button,"clicked",G_CALLBACK(aux_get_points_circle),drawing_area);
  button = gtk_button_new_with_label ("Get circle");
  gtk_box_pack_start (GTK_BOX (Hbox), button, C, D, E);
  gtk_widget_show (button); 
  g_signal_connect_swapped(button,"clicked",G_CALLBACK(button_press_event_circle),drawing_area);
  return Hbox;
}
